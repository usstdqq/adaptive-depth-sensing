...
def init_IDX_XY_grid(img_height, img_width, downsize, batch_size):
...
def build_LABXY_feat(label_in, XY_feat):
    ...
    curr_b, _, curr_img_height, curr_img_width = XY_feat.shape
    img_b, _, _, _ = img_lab.shape
    ...
    if curr_b == img_b:
        ...
        min_b = min(curr_b, img_b)
        LABXY_feat = torch.cat([scale_img[:min_b,:,:,:], XY_feat[:min_b,:,:,:]],dim=1)
    return LABXY_feat
